 ...
 */
export const isUnclaimed = (expense: Expense): boolean => !(expense.paidBy || '').trim();

export const isDarkCherry = (expense: Expense): boolean => expense.splitType === 'dark_cherry';

export const getDarkCherryPotTotal = (expense: Expense, includePending = true): number =>
  roundCurrency(
    (expense.settlements || [])
      .filter((s) => s.status !== 'voided' && (includePending || s.status === 'confirmed'))
      .reduce((total, s) => total + s.amount, 0)
  );

export const getDarkCherryRemaining = (expense: Expense, includePending = true): number =>
  Math.max(0, roundCurrency(expense.amount - getDarkCherryPotTotal(expense, includePending)));
...
export const isExpenseFullySettled = (expense: Expense): boolean => {
  ...
  const debtors = Object.keys(expense.shares || {}).filter((userId) => userId !== expense.paidBy);
  ...
  if (debtors.length > 0) {
    return debtors.every((userId) => getRemainingSettlementAmount(expense, userId, false) <= 0.01);
  }
...
export const getTotalRemainingOwedToPayer = (expense: Expense, includePending = false): number =>
  ...
  isUnclaimed(expense)
    ...
    : roundCurrency(
        Object.entries(expense.shares || {})
          ...
          .reduce(
            (total, [userId]) =>
              total + getRemainingSettlementAmount(expense, userId, includePending),
            0
          )
      );
