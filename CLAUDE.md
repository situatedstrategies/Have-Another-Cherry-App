...
- Client config lives in `firebase-applet-config.beta.json` (public identifiers, not
  secrets). It ships with `REPLACE_ME_*` placeholders; `src/firebase.ts` throws on
  startup if a beta build still has them, rather than falling back to production.
...
- `firestore.rules` deploys to both projects via the rules workflow; the beta leg stays
  dormant until `FIREBASE_SERVICE_ACCOUNT_BETA` and the real beta project id are set.
