...
const sum = (o: Record<string, number>, extra: { split: number }[] = []) =>
  Math.round(
    (Object.values(o).reduce((a, b) => a + b, 0) + extra.reduce((a, s) => a + s.split, 0)) * 10
  ) / 10;
...
check('third seat: pending seat carries the name and share', three.availableSplits, [
  { name: 'Casey', split: 30 },
]);
check(
  'third seat: the couple is scaled into the remaining 70, keeping their 60/40 ratio',
  three.defaultSplit,
  { a: 42, b: 28 }
);
...
check(
  'fourth seat: pending seats keep their order',
  four.availableSplits.map((s) => s.name),
  ['Casey', 'Dana']
);
...
check(
  'remove pending: only Dana is left pending',
  removed.availableSplits.map((s) => s.name),
  ['Dana']
);
check(
  'remove pending: total is still 100',
  sum(removed.defaultSplit, removed.availableSplits),
  100
);
...
const five = {
  ...
  memberIds: ['a', 'b', 'c', 'd', 'e'],
  members: [],
  defaultSplit: { a: 20, b: 20, c: 20, d: 20, e: 20 },
  targetNumPeople: 5,
};
...
check(
  'legacy group without a size falls back to five',
  groupCapacity({ ...couple, targetNumPeople: undefined }),
  5
);
...
const solo = {
  ...couple,
  members: [{ uid: 'a', name: 'A', email: '' }],
  memberIds: ['a'],
  defaultSplit: { a: 50 },
  availableSplits: [{ name: 'Partner', split: 50 }],
  targetNumPeople: undefined,
};
...
check(
  'a capacity of one already written reads as two',
  groupCapacity({ ...couple, memberIds: ['a'], targetNumPeople: 1 }),
  MIN_GROUP_CAPACITY
);
...
check(
  'scale to 70 keeps one decimal and sums exactly',
  scalePercents([33.3, 33.3, 33.4], 70),
  [23.3, 23.3, 23.4]
);
