...
const base = (over: Partial<Expense>): Expense => ({
  id: 'e', title: 'Rent', amount: 100, date: '2026-08-01', category: 'Rent',
  paidBy: 'alice', shares: { alice: 65, bob: 35 }, settlements: [], status: 'OPEN',
  ...over,
} as Expense);
...
check('confirmed payment settles it', isExpenseFullySettled(base({
  settlements: [{ id: 's', paidBy: 'bob', receivedBy: 'alice', amount: 35, instrumentType: 'VENMO', timestamp: '2026-08-02T00:00:00Z', status: 'confirmed' } as any],
})), true);
...
check('no shares, OPEN flag: not settled', isExpenseFullySettled(base({ shares: {}, status: 'OPEN' })), false);
check('no shares, CLOSED flag: settled', isExpenseFullySettled(base({ shares: {}, status: 'CLOSED' })), true);
