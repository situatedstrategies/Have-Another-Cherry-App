...
check(
  'an entry with no placeholder is returned as-is',
  claimSeatsOnExpense(after, ['rob', 'liv']) === after,
  true
);
...
check('both at once maps in join order', claimSeatsOnExpense(three, ['rob', 'liv', 'cas']).shares, {
  rob: 50,
  liv: 30,
  cas: 20,
});
...
check(
  'a seated member never takes a placeholder',
  seatClaimMap(exp({ shares: { rob: 50, liv: 30, ghost_0: 20 } }), ['rob', 'liv']),
  {}
);
...
check(
  'ledger pass: unchanged returns the same array',
  claimSeats(unchanged, ['rob', 'liv']).expenses === unchanged,
  true
);
check(
  'ledger pass: unchanged is not flagged as changed',
  claimSeats(unchanged, ['rob', 'liv']).changed,
  false
);
