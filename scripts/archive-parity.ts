...
eq(
  'no intra-day drift',
  iso(archiveCutoff(new Date(2026, 8, 2, 0, 1))) ===
    iso(archiveCutoff(new Date(2026, 8, 2, 23, 59)))
    ? 'same'
    : 'drifted',
  'same'
);
...
eq('sorts chronologically', ['2026-10', '2026-03', '2026-01'].sort(), [
  '2026-01',
  '2026-03',
  '2026-10',
]);
...
eq(
  'never archives recurring',
  s.recent.some((e: any) => e.id === 'rec'),
  true
);
eq(
  'never archives undated',
  s.recent.some((e: any) => e.id === 'u'),
  true
);
