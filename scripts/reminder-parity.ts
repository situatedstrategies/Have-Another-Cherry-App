...
check('a run reminds for tomorrow',
  reminderTargetDate(new Date('2026-09-05T21:00:00Z')), '2026-09-06');
check('it crosses a month boundary',
  reminderTargetDate(new Date('2026-09-30T21:00:00Z')), '2026-10-01');
check('it crosses a year boundary',
  reminderTargetDate(new Date('2026-12-31T21:00:00Z')), '2027-01-01');
check('it handles a leap day',
  reminderTargetDate(new Date('2028-02-28T21:00:00Z')), '2028-02-29');
...
check('an evening run in a western zone still means tomorrow there',
  reminderTargetDate(new Date('2026-09-06T01:00:00Z'), -4), '2026-09-06');
check('the same instant with no offset is a day further out',
  reminderTargetDate(new Date('2026-09-06T01:00:00Z'), 0), '2026-09-07');
check('a positive offset works too',
  reminderTargetDate(new Date('2026-09-05T22:00:00Z'), 9), '2026-09-07');
...
check('one bill due carries its id and target',
...
check('several bills due omit the id, since one push cannot point at three',
...
check('a recurring expense carries its own target',
...
check('every value is a string, whatever the entry held',
  Object.values(reminderPayload('ABC123', '2026-09-10', [{ id: 42, target: null }]))
...
// claim is that it says nothing about what the bill is or what it costs.
{
  ...
  check('and it carries only the four agreed keys',
    Object.keys(payload).sort(), ['dueDate', 'groupId', 'id', 'target', 'type'].sort());
}
